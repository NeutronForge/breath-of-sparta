...
public class CustomWindChargeItem extends Item implements ProjectileItem{

    public CustomWindChargeItem(Item.Settings settings) {
        super(settings);
    }
    ...
    public ActionResult use(World world, PlayerEntity user, Hand hand) {
        ...
        if (world instanceof ServerWorld serverWorld) {
            ProjectileEntity.spawnWithVelocity(
                    (world2, shooter, stack) -> new CustomWindChargeEntity(user, world, user.getPos().getX(), user.getEyePos().getY(), user.getPos().getZ()),
                    serverWorld,
                    itemStack,
                    user,
                    0.0F,
    ...
    public ProjectileEntity createEntity(World world, Position pos, ItemStack stack, Direction direction) {
        ...
        CustomWindChargeEntity customWindChargeEntity = new CustomWindChargeEntity(world, pos.getX(), pos.getY(), pos.getZ(), vec3d);
        customWindChargeEntity.setVelocity(vec3d);
        return customWindChargeEntity;
    }
